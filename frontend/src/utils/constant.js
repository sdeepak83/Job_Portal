export const USER_END_POINT =
  "https://job-portal-backend-vvjj.onrender.com/api/v1/user";

export const JOB_API_END_POINT =
  "https://job-portal-backend-vvjj.onrender.com/api/v1/job";

export const APPLICATION_API_END_POINT =
  "https://job-portal-backend-vvjj.onrender.com/api/v1/application";

export const COMPANY_API_END_POINT =
  "https://job-portal-backend-vvjj.onrender.com/api/v1/company";

// export const USER_END_POINT = "p://localhost:8000/api/v1/user";
// htt
// export const JOB_API_END_POINT = "http://localhost:8000/api/v1/job";

// export const APPLICATION_API_END_POINT =
//   "http://localhost:8000/api/v1/application";

// export const COMPANY_API_END_POINT = "http://localhost:8000/api/v1/company";
